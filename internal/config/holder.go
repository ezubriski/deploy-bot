...
// Reload re-reads the config file. On success it atomically replaces the
// current Config and returns the new value. On any error it leaves the
// existing Config unchanged and returns a wrapped error — callers should
// treat this as a no-op warning, not a fatal condition.
